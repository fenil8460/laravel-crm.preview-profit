    ...
    <table id="example" class="display" style="width:100%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <form action="/useful_links/showlinks" method="post" id="statusForm{{$data->id}}">
                    @csrf
                    <!-- If status is 1 an unchecked checkbox -->
                    @if ($data->Visible == "hide")
                    <td><a href="/useful_links/showlinks/{{$data->id}}">
                            <div class="btn-group btn-group-xs">
                                <button type="button" class="btn btn-primary">Show</button>
                            </div>
                        </a>
                        <a href='/useful_links/edit/{{ $data->id }}'>🖉Edit</a>
                        <a href='/useful_links/delete/{{ $data->id }}'>❌Del</a>
                    </td>
                    <input name="id" type="hidden" value="{{$data->id}}">

                    @else
                    <td style="border-left-style: unset;border-right-style: unset;"><a
                            href="/useful_links/hidelinks/{{$data->id}}">
                            <div class="btn-group btn-group-xs">
